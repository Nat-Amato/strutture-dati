...
void Linked_list<T>::invert() {
	position pLeft = begin();
	position pRight = last();
	value_type vLeft, vRight;
	...
	do {
		...
		pLeft = next(pLeft);
		pRight = previous(pRight);
	} while (previous(pLeft) != next(pRight) && previous(pLeft) != pRight);
}
...
bool Linked_list<T>::palindrome() {
	...
	do {
		if (read(pLeft) != read(pRight))
			verifica = false;

		pLeft = next(pLeft);
		pRight = previous(pRight);
	} while (previous(pLeft) != next(pRight) && previous(pLeft) != pRight && verifica == true);
	return verifica;
}
